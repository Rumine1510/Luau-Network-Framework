...
function NetworkPrototype.__RegisterSignal(self: network, eventName: string, property: signalProperty, eventID: number): customSignal
...
function NetworkPrototype.__AddQueue(self: network, key: string, params: signalParams, args: array)
...
function NetworkPrototype.__ReplicateStorage(self: network, key: string, value: any)

	local target = self.ConnectedClient

	if type(target) == "table" and #target > 0 then

		self:__AddQueue('', {Target = target}, {key, value})
	...
	if IsServer then
		...
		self._trove:Set(self.__MainInvoke, "OnServerInvoke", function(player, key, ...): any
			...
			if not key then
				...
				local signalInfoArray = returnTable[1]
				...
				if type(self.ReplicatedStorage) == "table" then
					...
					returnTable[2] = replicatedStorage
					
					for k,v in self.ReplicatedStorage do
		...
		if self.__MainFolder then
			...
			if initialiseData then
				...
				if typeof(self.ReplicatedStorage) == "table" then
					
					for k,v in initialiseData[2] do
						self.ReplicatedStorage[k] = v
					end
				...
				if data[''] then
					...
					for _, args in data[''] do
						...
						elseif argsN == 2 then
							...
							local key, value = table.unpack(args)
							assert(type(self.ReplicatedStorage) == "table", "Network's ReplicatedStorage is not enabled.")					
							
							self.ReplicatedStorage[key] = value
							
						elseif argsN == 3 then

							self:CreateSignal(table.unpack(args))
...
function NetworkPrototype.ConnectSignal(self: network, eventName: string, func: func): customConnection
...
function NetworkPrototype.InvokeSignal(self: network, eventName: string, params: signalParams?, ...: any): any
...
function NetworkPrototype.ConnectLocalSignal(self: network, eventName: string, func: func): customConnection
...
function NetworkPrototype.InvokeLocalSignal(self: network, eventName: string, params: signalParams?, ...: any): any
...
function NetworkPrototype.Reconnect(self: network)
	...
	if self.Property.AllowCrossCommunication then
		...
		if self.Property.EnableReplicatedStorage then
			...
			if IsServer then
				...
				tbl = EventTable.new(function(key, oldValue, newValue)
					self:__ReplicateStorage(key, newValue)
				end)

				self.ReplicatedStorage = tbl
